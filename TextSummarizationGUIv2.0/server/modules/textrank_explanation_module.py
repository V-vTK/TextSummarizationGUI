...
import re
class TextRankExplanationModule(BaseExplanationModule):
    def __call__(self, input_value):
        document = self.parser.document
        ratings = self.summarizer.rate_sentences(document)
        matrix = self.summarizer._create_matrix(document)
        ...
        power_method = self.summarizer.power_method(matrix, 1e-4)
        result = {
